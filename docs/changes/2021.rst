...
Internal optimizations:  New :term:`django-admin command` :manage:`demotest`
increases test coverage. Removed some DeprecationWarnings.
:meth:`lino_react.react.views.ApiElement.post` no longer passes
`http.QueryDict(request.body)` but `request.POST` as data to
